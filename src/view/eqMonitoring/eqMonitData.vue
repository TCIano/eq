  ...
  <div class="eq-detail">
    ...
    >
      <a-tab-pane key="1" tab="设备参数">
        <eq-param :data="tableData" :equipment_id="equipment_id"></eq-param>
      </a-tab-pane>
...
export default {
  ...
  methods: {
    getEquipmentImage(picture) {
      try {
        return require('../../../assets/equipment/' + (picture || '卧罐.png'))
      } catch (e) {
        return require('../../assets/equipment/卧罐.png')
      }
    },
    onBack() {
      this.$router.push({
    ...
    async getComprehensiveMonitor() {
      ...
      if (result) {
        ...
        for (let index = 0; index < this.mapList.length; index++) {
          this.mapList[index].list = result.filter(item => item[this.mapList[index].key]).map(pn => {
            return {
